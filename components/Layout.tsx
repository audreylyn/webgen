      ...
      <aside className="w-64 bg-indigo-900 text-white flex flex-col fixed h-full z-10">
        ...
        <div className="p-4 border-t border-indigo-800 space-y-4">
           ...
           <div className="bg-indigo-800 p-3 rounded-lg">
             <label className="block text-xs text-indigo-300 mb-2 font-semibold uppercase">Simulate Role</label>
             <div className="flex gap-2">
               <button 
                onClick={() => switchRole('admin')}
                className={`flex-1 text-xs py-1 rounded ${user.role === 'admin' ? 'bg-indigo-500 text-white shadow' : 'text-indigo-300 hover:bg-indigo-700'}`}
               >
                 Admin
               </button>
               <button 
                onClick={() => switchRole('editor')}
                className={`flex-1 text-xs py-1 rounded ${user.role === 'editor' ? 'bg-indigo-500 text-white shadow' : 'text-indigo-300 hover:bg-indigo-700'}`}
               >
                 Editor
               </button>
             </div>
